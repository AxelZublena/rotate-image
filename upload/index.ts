...
const httpTrigger: AzureFunction = async function(context: Context, req: HttpRequest): Promise<any> {
	...
	if (!req.query?.rotation) {
		context.res.body = `rotation degree is not defined`;
		context.res.status = HTTP_CODES.BAD_REQUEST
	}
	...
	try {
		...
		context.log(parseFloat(req.query?.rotation))
		let file = parts[0].data;
		const processedFile = await sharp(file).rotate(parseFloat(req.query?.rotation)).png().toBuffer();

		// Passed to Storage
		// context.bindings.storage = parts[0]?.data;
		context.bindings.storage = processedFile;

		// returned to requestor
		context.res.body = `${req.query?.username}/${new Date().getTime().toString()}.png`;
	} catch (err) {
